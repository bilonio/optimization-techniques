...
for i = 1:length(formats)
    mkdir(fullfile('../figures', 'ex2', sprintf('%s', formats(i))));
end
...
for i = length(x0):-1:1
    ...
    for j = 1:length(celoptionarray)
        ...
        [res] = steepestDescent(1e-4, x0(i,:), f, celoptionarray(j));

        results(ctr).xmin = res.xmin;
        results(ctr).dist = res.dist;
        results(ctr).gammas = res.gammas;
        results(ctr).time = res.time;
        results(ctr).xs = res.xs;
        results(ctr).k = res.k;
        results(ctr).method = res.method;
        results(ctr).x0 = x0(i,:);
    ...
    for k  = 1:length(formats)
         ...
         delete(fullfile('../figures', 'ex2', sprintf("%s", formats(k)), sprintf("ex2a(%.0f, %.0f).%s", x0(i,1), x0(i,2), formats(k))));
         saveas(fig, fullfile('../figures', 'ex2', sprintf("%s", formats(k)), sprintf("ex2a(%.0f, %.0f).%s", x0(i,1), x0(i,2), formats(k))));
    end
end
