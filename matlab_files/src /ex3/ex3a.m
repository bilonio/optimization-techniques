...
for i = length(x0):-1:1
    ...
    for j = 1:length(celoptionarray)
        ...
        [res] = newton(1e-4, x0(i,:), f, celoptionarray(j));
    ...
    for k  = 1:length(formats)
         ...
         delete(fullfile('../../figures', 'ex3', sprintf("%s", formats(k)), sprintf("ex3a(%.0f, %.0f).%s", x0(i,1), x0(i,2), formats(k))));
         saveas(fig, fullfile('../../figures', 'ex3', sprintf("%s", formats(k)), sprintf("ex3a(%.0f, %.0f).%s", x0(i,1), x0(i,2), formats(k))));
    end
end
