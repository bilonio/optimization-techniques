...
legend;  
    ...
    for k  = 1:length(formats)
         ...
         delete(fullfile('../../figures', 'ex2', sprintf("%s", formats(k)), sprintf("ex2a(%.0f, %.0f).%s", x0(1,1), x0(1,2), formats(k))));
         saveas(fig, fullfile('../../figures', 'ex2', sprintf("%s", formats(k)), sprintf("ex2a(%.0f, %.0f).%s", x0(1,1), x0(1,2), formats(k))));
    end
